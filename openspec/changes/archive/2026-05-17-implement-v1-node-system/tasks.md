...
- [x] 2.1 Implement and test `std/manual-ignite`, `std/transmute`, `std/accumulation`, `std/filter`, `std/merge`, and `std/manual-accept`.
- [x] 2.2 Implement and test `debug/log`, `debug/placeholder-echo`, `debug/placeholder-rhai`, and `debug/note`.
- [x] 2.3 Implement and test `std/sleep`, `std/cron`, `std/freezer`, `std/cache`, and `codex/exec`.

## 3. Authoring And Fixtures

- [x] 3.1 Update the Orchidex node authoring skill with v1 Rhai host API and shell delegation examples.
- [x] 3.1.a Document port definitions and default-port migration behavior in node authoring guidance.
- [x] 3.2 Add fixture-first Codex and cache/freezer acceptance tests.
- [x] 3.3 Run `cargo test --workspace` and `bun run core -- run-scenario`.
